...
def setup(get_artworks=False, get_card_images=False, get_archetypes=False, map_archetypes=False):
    if get_artworks: get_all_card_images(get_artworks=True)
    if get_card_images: get_all_card_images(get_artworks=False)
    if get_archetypes: get_all_archetypes()
    if map_archetypes: get_archetype_members()
    populate_database()
    return
...
def get_archetype_members():
    ...
    for arch in archetypes:
        try:
            request = requests.get(f"https://yugipedia.com/api.php?action=askargs&conditions=Archseries::{arch}]]OR[[{arch.lower()}&format=json", headers=headers).json()
